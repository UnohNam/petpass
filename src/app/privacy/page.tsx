...
const ROWS: [string, string][] = [
  ["수집 항목", "아이디, 비밀번호(암호화되어 저장되며 운영자도 볼 수 없음), 이용자가 입력한 반려동물 정보(이름, 종, 견종, 체중, 이동장·유모차 보유 여부, 맹견 여부), 저장한 장소 목록과 준비물 체크 상태"],
  ...
  ["제3자 제공", "제공하지 않습니다."],
  ["수집하지 않는 것", "이메일, 전화번호, 실명은 받지 않습니다. 그래서 비밀번호를 잊으면 찾을 수 없습니다. 위치 정보는 '내 주변' 검색 시 브라우저에서 한 번 읽어 장소 조회에만 쓰고 저장하지 않습니다. 로그인하지 않으면 모든 정보는 이용자의 브라우저에만 저장됩니다."],
